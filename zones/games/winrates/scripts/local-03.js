...
async function fncDealCards()
{
  ...
  // Get all keys from arrWinRates
  const keys = Object.keys(arrWinRates);

  // Validate subset range
  const validStart = Math.max(0, numRangeBegin);
  const validEnd = Math.min(numRangeEnd, keys.length - 1);
  ...
  strHoleValue = arrWinRates[strHoleKey];  // Save to global variable

  // Initialize card strings
  arrCards[0] = '';
  arrCards[1] = '';

  // Add the first character of strHoleKey to arrCards[0]
  arrCards[0] += strHoleKey.charAt(0);
  // Add the second character of strHoleKey to arrCards[1]
  ...
  const shuffledSuits = [...arrSuits].sort(() => Math.random() - 0.5);
  ...
  if (suitType === 's') {
    ...
    arrCards[0] += shuffledSuits[0];
    arrCards[1] += shuffledSuits[1];
