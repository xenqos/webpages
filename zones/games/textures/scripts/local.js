...
//    objConsoleLog.innerHTML += ">>> Hand Tested: " + arrCards + `\n\n`;
    ...
    else // No ranks with pair or three on board
    {
        ...
        else // Hole cards are not a pair (e.g., the T7 case)
        {
            // No Pair to Pair: 6 outs (3 for Card 1 + 3 for Card 2)
            numFlopOutsNoPairPair = 6;

            // NOTE: Logic for 'One Overcard to Pair' may also be here. If so, it should be removed
            // or zeroed out, as 'No Pair to Pair' covers all pairing outs.
        }
    }
...
//    strOutsTable += `<tr><td colspan=2>&nbsp;</td></tr>\n`;
    ...
    else if (numTurnRanksWithPair === 1)
    {
        if (numHoleCard1Rank !== numHoleCard2Rank)
        {
            // One Pair to Two Pair outs (3 for the other hole card)
            let unpairedHoleCardRank = -1;
            if (arrTurnRankCounts[numHoleCard1Rank] === 1)
            {
                unpairedHoleCardRank = numHoleCard1Rank;
            }
            else if (arrTurnRankCounts[numHoleCard2Rank] === 1)
            {
                unpairedHoleCardRank = numHoleCard2Rank;
            }

            if (unpairedHoleCardRank !== -1)
            {
                numTurnOutsOnePairTwoPair = 3;
            }
        }
    ...
    for (numIdxRank = 2; numIdxRank <= 10; numIdxRank++)
    {
        ...
        if (numCardsInRun === 4 && numGaps === 1)
        {
            // Check if the single missing card (gap) is at either end (OESD) or inside (ISD).
            if (arrTurnStraightRanks[numIdxRank] === 0 || arrTurnStraightRanks[numIdxRank + 4] === 0)
            {
                // Gap is at the end (e.g., X, 4, 5, 6, 7 or 4, 5, 6, 7, X) -> OESD
                numTurnOutsOESD = 8;
            }
            else
            {
                // Gap is inside (e.g., 4, 5, X, 7, 8) -> ISD
    ...
    // --- REDUNDANCY CATCH (Final filtering before sum) ---
    // If any strong draw (Straight or Flush) is present, zero out the highly redundant
    // 'One Overcard to Pair' outs, as they are often incorrectly set or covered by 'No Pair to Pair'.
    if (numTurnOutsOESD > 0 || numTurnOutsInsideStraight > 0 || numTurnOutsOESDFlushDraw > 0 || numTurnOutsInsideStraightFlush > 0 || numTurnOutsFlushDraw > 0)
    {
        numTurnOutsOvercardPair = 0; // Eliminates the incorrect 3 outs for the 7-high hand
    }
    // ---------------------------------------------------
    ...
    numTurnOutsTotal += numTurnOutsOnePairTwoPair;
