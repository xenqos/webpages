...
function fncPlayPhoneme(objPhoneme)
{
//  console.log("Hello world!");

  var varToneSelected = 0;
  var varSound = '';
  var varGrapheme = objPhoneme.innerHTML;
  var arrPhonemes = arrPinyin[varGrapheme].split(',');

  for (var i = 0; i < document.tones.tone.length; i++)
  {
    if (document.tones.tone[i].checked)
    {
      varToneSelected = document.tones.tone[i].value;
    }
  }

  if (varToneSelected != 5)
  {
    fncPlaySound(arrPhonemes[varToneSelected - 1]);
  }
  else
  {
    fncPlaySound(arrPinyin[varGrapheme]);
  }
}
